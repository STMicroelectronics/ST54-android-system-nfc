...
/*******************************************************************************
**
** Function         nfa_dm_change_poll_mask
**
...
static uint8_t nfa_dm_get_rf_discover_config(
  ...
  if (nfa_dm_cb.flags & NFA_DM_FLAGS_LISTEN_DISABLED) {
    LOG(DEBUG) << StringPrintf("%s; listen disabled, rm listen from 0x%x",
  ...
  if (nfa_dm_cb.flags & NFA_DM_FLAGS_POLL_TECH_CHANGED) {
    /* Check polling tech */
    LOG(DEBUG) << StringPrintf("%s; poll tech will be changed to 0x%x",
    ...
    LOG(DEBUG) << StringPrintf("%s; poll tech will set to 0x%x", __func__,
                               (dm_disc_mask & NFA_DM_DISC_MASK_POLL));
  }
...
static tNFA_STATUS nfa_dm_set_rf_listen_mode_config(
  ...
  LOG(DEBUG) << StringPrintf("%s; tech_proto_mask = 0x%08X", __func__,
                             tech_proto_mask);
  ...
  */
  ...
  if ((nfa_dm_cb.custom_config == false) && (appl_dta_mode_flag == 0)) {
...
static void nfa_dm_set_rf_listen_mode_raw_config(
  ...
  *p_disc_mask = disc_mask;
  ...
  LOG(DEBUG) << StringPrintf("%s; disc_mask = 0x%x", __func__, disc_mask);
}
...
static tNFC_STATUS nfa_dm_disc_force_to_idle(void) {
  ...
  LOG(DEBUG) << StringPrintf("%s; disc_flags = 0x%x", __func__,
...
void nfa_dm_start_rf_discover(void) {
  ...
  LOG(DEBUG) << StringPrintf("%s; dm_disc_mask = 0x%x", __func__, dm_disc_mask);
...
bool nfa_dm_disc_handle_kovio_activation(tNFC_DISCOVER* p_data,
  ...
  if (nfa_dm_cb.disc_cb.kovio_tle.in_use) {
    ...
    if ((nfa_dm_cb.activated_nfcid_len !=
      ...
      LOG(DEBUG) << StringPrintf("%s; new Kovio tag is detected", __func__);
...
static tNFA_STATUS nfa_dm_disc_notify_activation(tNFC_DISCOVER* p_data) {
  ...
  LOG(DEBUG) << StringPrintf("%s; tech_n_mode:0x%X, proto:0x%X", __func__,
...
static void nfa_dm_disc_notify_deactivation(tNFA_DM_RF_DISC_SM_EVENT sm_event,
  ...
  LOG(DEBUG) << StringPrintf("%s; activated_handle=%d", __func__,
                             nfa_dm_cb.disc_cb.activated_handle);

  if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_CHECKING) {
    LOG(DEBUG) << StringPrintf("%s; for sleep wakeup", __func__);
    return;
  }
...
static void nfa_dm_disc_sm_idle(tNFA_DM_RF_DISC_SM_EVENT event,
  ...
  switch (event) {
    ...
    case NFA_DM_RF_INTF_ACTIVATED_NTF:
      /// RFST misalignment between stack and NFCC, ask for restart
      (*nfa_dm_cb.p_dm_cback)(NFA_DM_NFCC_TRANSPORT_ERR_EVT, nullptr);
      break;
    ...
    case NFA_DM_RF_INTF_ACTIVATED_NTF:
      if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_RSP) {
        ...
        if (nfa_rw_cb.mifare_pres_check_status ==
          ...
          if (p_data->nfc_discover.activate.protocol != NFA_PROTOCOL_MIFARE) {
            nfa_rw_cb.mifare_pres_check_status = NFA_RW_MIFARE_PRES_CHECK_NONE;
            nfa_dm_disc_end_sleep_wakeup(NFA_STATUS_FAILED);
          } else {
      ...
      if (nfa_dm_cb.disc_cb.disc_flags & NFA_DM_DISC_FLAGS_W4_RSP) {
        ...
        // Rx DEACTIVATE_NTF(discovery)
          ...
          if (nfa_dm_cb.listen_deact_cmd_type ==
              NFC_DEACTIVATE_TYPE_DISCOVERY) {
            // If receving DEACT_CMD(disc) while in RFST_DISCOVERY
...
static void nfa_dm_disc_sm_lp_listen(tNFA_DM_RF_DISC_SM_EVENT event,
  ...
  switch (event) {
    case NFA_DM_RF_INTF_ACTIVATED_NTF:
      ...
      if (nfa_dm_disc_notify_activation(&(p_data->nfc_discover)) ==
          NFA_STATUS_FAILED) {
        LOG(DEBUG) << StringPrintf("%s; Not matched, unexpected activation",
                                   __func__);
      }
      break;
...
void nfa_dm_delete_rf_discover(tNFA_HANDLE handle) {
  LOG(DEBUG) << StringPrintf("%s; handle=0x%x", __func__, handle);
...
tNFA_STATUS nfa_dm_rf_deactivate(tNFA_DEACTIVATE_TYPE deactivate_type) {
  LOG(DEBUG) << StringPrintf("%s; deactivate_type:0x%X", __func__,
                             deactivate_type);
  ...
  if (nfa_dm_cb.disc_cb.disc_state == NFA_DM_RFST_IDLE) {
    ...
    if (deactivate_type == NFA_DEACTIVATE_TYPE_DISCOVERY) {
      if (nfa_dm_cb.disc_cb.kovio_tle.in_use) {
        ...
        LOG(DEBUG) << StringPrintf("%s; already in discovery state", __func__);
        return NFA_STATUS_FAILED;
      }
...
void nfa_dm_get_tech_route_block(uint8_t* listen_techmask, bool* enable) {
  ...
  if (*enable) {
    LOG(INFO) << StringPrintf("%s; change_listen_mask: 0x%x, ", __func__,
                              *listen_techmask);
  }
}
