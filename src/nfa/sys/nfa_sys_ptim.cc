...
void nfa_sys_ptim_timer_update(tPTIM_CB* p_cb) {
  ...
  while (!GKI_timer_list_empty(&p_cb->timer_queue) &&
    ...
    LOG(DEBUG) << StringPrintf("%s; expired: %p", __func__, p_tle);
...
void nfa_sys_ptim_start_timer(tPTIM_CB* p_cb, TIMER_LIST_ENT* p_tle,
  ...
  LOG(DEBUG) << StringPrintf("%s; %p", __func__, p_tle);

  /* if timer list is currently empty, start periodic GKI timer */
  if (GKI_timer_list_empty(&p_cb->timer_queue)) {
    LOG(DEBUG) << StringPrintf("%s; Start timer_id: %d", __func__,
                               p_cb->timer_id);
...
void nfa_sys_ptim_stop_timer(tPTIM_CB* p_cb, TIMER_LIST_ENT* p_tle) {
  ...
  /* if timer list is empty stop periodic GKI timer */
  if (GKI_timer_list_empty(&p_cb->timer_queue)) {
    LOG(DEBUG) << StringPrintf("%s; stop timer_id: %d", __func__,
                               p_cb->timer_id);
    GKI_stop_timer(p_cb->timer_id);
  }
}
