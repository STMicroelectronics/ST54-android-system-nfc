  ...
  if (nfc_status == NCI_STATUS_OK) {
    ...
    if (nfc_cb.nci_version >= NCI_VERSION_2_0) {
      ...
      if (evt_data.enable.nci_features & NCI_FEAT_HCI_NETWORK) {
        ...
        LOG(DEBUG) << StringPrintf("%s; hci num_buf=%d buf_size=%d", __func__,
...
void nfc_set_state(tNFC_STATE nfc_state) {
  LOG(DEBUG) << StringPrintf("%s; %d (%s)->%d (%s)", __func__, nfc_cb.nfc_state,
...
void nfc_main_handle_hal_evt(tNFC_HAL_EVT_MSG* p_msg) {
  ...
  LOG(DEBUG) << StringPrintf("%s; HAL event=0x%x", __func__, p_msg->hal_evt);
...
static void nfc_main_hal_cback(uint8_t event, tHAL_NFC_STATUS status) {
  LOG(DEBUG) << StringPrintf("%s; event: %s(0x%x), status=%d", __func__,
  ...
  switch (event) {
    ...
    default:
      LOG(DEBUG) << StringPrintf("%s; unhandled event %x", __func__, event);
      break;
  }
}
...
void NFC_Disable(void) {
  LOG(DEBUG) << StringPrintf("%s; nfc_state = %d", __func__, nfc_cb.nfc_state);
...
tNFC_STATUS NFC_DiscoveryMap(uint8_t num, tNFC_DISCOVER_MAPS* p_maps,
  ...
  num_intf = 0;
  LOG(DEBUG) << StringPrintf("%s; nci_interfaces supported by NFCC: 0x%x",
  ...
  for (xx = 0; xx < num_disc_maps; xx++) {
    ...
    if (p_maps[xx].intf_type > NCI_INTERFACE_MAX) {
      ...
      LOG(DEBUG) << StringPrintf("%s; [%d]: vs intf_type:0x%x is_supported:%d",
                                 __func__, xx, p_maps[xx].intf_type,
                                 is_supported);
    } else {
...
tNFC_STATUS NFC_Deactivate(tNFC_DEACT_TYPE deactivate_type) {
  ...
  if (nfc_cb.nfc_state == NFC_STATE_OPEN) {
    ...
    LOG(DEBUG) << StringPrintf("%s; act_protocol %d credits:%d/%d", __func__,
