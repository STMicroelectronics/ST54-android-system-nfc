...
uint8_t nfc_ncif_send_data(tNFC_CONN_CB* p_cb, NFC_HDR* p_data) {
  ...
  LOG(DEBUG) << StringPrintf("%s; conn_id: %d, num_buff:%d qc:%d", __func__,
...
bool nfc_ncif_process_event(NFC_HDR* p_msg) {
  ...
  switch (mt) {
    case NCI_MT_DATA:
      LOG(DEBUG) << StringPrintf("%s; NFC received data", __func__);
      nfc_ncif_proc_data(p_msg);
      free = false;
      break;

    case NCI_MT_RSP:
      LOG(DEBUG) << StringPrintf("%s; NFC received rsp gid:%d", __func__, gid);
    ...
    case NCI_MT_NTF:
      LOG(DEBUG) << StringPrintf("%s; NFC received ntf gid:%d", __func__, gid);
      switch (gid) {
        case NCI_GID_CORE: /* 0000b NCI Core group */
    ...
    default:
      LOG(DEBUG) << StringPrintf("%s; NFC received unknown mt:0x%x, gid:%d",
                                 __func__, mt, gid);
  }
...
void nfc_ncif_proc_activate(uint8_t* p, uint8_t len) {
  ...
  len_act = *p++;
  LOG(DEBUG) << StringPrintf("%s; len: %d len_act: %d, mode:0x%02x", __func__,
                             len, len_act, mode);
...
void nfc_ncif_proc_ee_discover_req(uint8_t* p, uint16_t plen) {
  ...
  LOG(DEBUG) << StringPrintf("%s; %d len:%d", __func__, *p, plen);
  ...
  if (p_cback) {
    ...
    while ((u8 > 0) && (plen >= NFC_EE_DISCOVER_ENTRY_LEN)) {
      ...
      {
        LOG(DEBUG) << StringPrintf("%s; bad entry len:%d", __func__, *p);
        return;
      }
      p++;
      /* V */
...
void nfc_ncif_proc_reset_rsp(uint8_t* p, bool is_ntf) {
  ...
  if (*p_len > 2 && is_ntf) {
    ...
    if ((status == NCI2_X_RESET_TRIGGER_TYPE_CORE_RESET_CMD_RECEIVED ||
      ...
      LOG(DEBUG) << StringPrintf("%s; CORE_RESET_NTF nci_version%x", __func__,
                                 nfc_cb.nci_version);
      ...
      if (nfc_cb.p_resp_cback) {
        ...
        LOG(DEBUG) << StringPrintf("%s; CORE_RESET_NTF restarting", __func__);
        ncif_fw_reset = false;
        return;
      }
    ...
    LOG(DEBUG) << StringPrintf("%s; CORE_RESET_RSP len :0x%x ", __func__,
                               *p_len);
...
void nfc_ncif_proc_init_rsp(NFC_HDR* p_msg) {
  ...
  if (status == NCI_STATUS_OK) {
    if (nfc_cb.nci_version == NCI_VERSION_UNKNOWN) {
      ...
      LOG(DEBUG) << StringPrintf("%s; scbr support: 0x%x", __func__,
...
void nfc_ncif_proc_data(NFC_HDR* p_msg) {
  ...
  if (p_cb && (p_msg->len >= NCI_DATA_HDR_SIZE)) {
    LOG(DEBUG) << StringPrintf("%s; 0x%02x%02x%02x, len:%d", __func__, pp[0],
                               pp[1], pp[2], len);
    ...
    if (p_last && (p_last->layer_specific & NFC_RAS_FRAGMENTED)) {
      ...
      if (!(p_last->layer_specific & NFC_RAS_TOO_BIG)) {
        ...
        LOG(DEBUG) << StringPrintf("%s; len:%d", __func__, p_last->len);
