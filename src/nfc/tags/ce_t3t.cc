  ...
  for (i = 0; i < p_cb->cur_cmd.num_blocks; i++) {
    ...
    if (service_code == T3T_MSG_NDEF_SC_RW) {
      if (p_cb->cur_cmd.num_blocks > p_cb->ndef_info.nbw) {
        LOG(ERROR) << StringPrintf(
            "%s; Requested too many blocks to update (requested: %i, max: "
            "%i)",
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s; Update sc 0x%04x block %i.", __func__,
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s; Udpate sc 0x%04x block %i.", __func__,
                            service_code, block_number);
        ...
        if (block_number > p_cb->ndef_info.nmaxb) {
          /* Error: invalid block number to update */
          LOG(ERROR) << StringPrintf(
              "%s; Requested invalid NDEF block number to update %i (max "
              "is "
              "%i).",
    ...
    for (i = 0; i < p_cb->cur_cmd.num_blocks; i++) {
      ...
      if ((service_code == T3T_MSG_NDEF_SC_RO) ||
        ...
        if (p_cb->cur_cmd.num_blocks > p_cb->ndef_info.nbr) {
          ...
          LOG(ERROR) << StringPrintf(
              "%s; Requested too many blocks to check (requested: %i, "
              "max: %i)",
          ...
          if (block_number > p_cb->ndef_info.nmaxb) {
            ...
            LOG(ERROR) << StringPrintf(
                "%s; Requested block number to check %i.", __func__,
                block_number);
        ...
        /* Error: invalid service code */
        LOG(ERROR) << StringPrintf(
            "%s; Requested invalid service code: 0x%04x.", __func__,
            service_code);
...
void ce_t3t_data_cback(tNFC_DATA_CEVT* p_data) {
  ...
  if (p_msg->len < 2) {
    ...
    if (cmd_type == CE_T3T_COMMAND_INVALID) {
      LOG(ERROR) << StringPrintf(
          "%s; received invalid T3t message (invalid command: 0x%02X)",
          __func__, cmd_id);
...
tNFC_STATUS CE_T3tSetLocalNDEFMsg(bool read_only, uint32_t size_max,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; ro=%i, size_max=%i, size_current=%i", __func__,
...
tNFC_STATUS CE_T3tSetLocalNDefParams(uint8_t nbr, uint8_t nbw) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; nbr=%i, nbw=%i", __func__, nbr, nbw);
...
tNFC_STATUS CE_T3tSendCheckRsp(uint8_t status1, uint8_t status2,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; status1=0x%02X, status2=0x%02X, num_blocks=%i",
                      __func__, status1, status2, num_blocks);
...
tNFC_STATUS CE_T3tSendUpdateRsp(uint8_t status1, uint8_t status2) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s; status1=0x%02X, status2=0x%02X", __func__, status1, status2);
