...
#include <com_android_nfc_st_nci_flags.h>
#include <log/log.h>
#include <string.h>
...
using com::android::nfc::st::nci::flags::t5t_no_getsysinfo;
...
tNFC_STATUS rw_i93_send_cmd_inventory(uint8_t* p_uid, bool including_afi,
  ...
  LOG(DEBUG) << StringPrintf("%s; including_afi:%d, AFI:0x%02X", __func__,
                             including_afi, afi);
...
void rw_i93_sm_read_ndef(NFC_HDR* p_resp) {
  ...
  if (flags & I93_FLAG_ERROR_DETECTED) {
    LOG(DEBUG) << StringPrintf("%s; Got error flags (0x%02x)", __func__, flags);
    rw_i93_handle_error(NFC_STATUS_FAILED);
    return;
  }
...
tNFC_STATUS RW_I93ReadSingleBlock(uint16_t block_number) {
  ...
  LOG(DEBUG) << StringPrintf("%s; block_number:0x%02X", __func__, block_number);
...
tNFC_STATUS RW_I93SetAddressingMode(bool mode) {
  LOG(DEBUG) << StringPrintf("%s; I93 state:%d", __func__, rw_cb.tcb.i93.state);
